...
mod tests {
    ...
    /// Same as the Metal case above, for a `--features cuda` build. On a build
    /// without the feature (or a machine with no CUDA-capable GPU), asking must
    /// fail rather than quietly give CPU.
    #[test]
    fn naming_cuda_matches_what_the_build_has() {
        let asked = resolve_device(Some("cuda"));
        assert_eq!(asked.is_ok(), candle_core::utils::cuda_is_available());
    }
