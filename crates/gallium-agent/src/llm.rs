...
/// SSE decoding for the streaming Responses API (`stream: true`). A child module
/// so it can use the private wire types below without widening their visibility.
mod sse;
...
impl OpenAiProvider {
    ...
    fn send_request_streaming(
        ...
        let response = sse::parse_sse_stream(reader, on_delta)?;
        Self::require_complete(response)
    }
