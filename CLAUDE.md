...
| `probe.rs` | `hidden` — per-stage forward-pass fingerprints on the `gallium::layers` trace target, diffed across devices (and against an f32 reference via `GALLIUM_LAYERS_F32_REF`) by `scripts/layer_diff.py`; see docs/CANDLE_BACKEND.md §6d–§6f |
